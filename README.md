<b>Update 9/14/2019</b>: For some reason, the module `playerctl.py` for py3status is outputting multiple istances of the error `No players found` in `.xsession-errors`.  It can still be used but it is still buggy.  A solution is on the way so please be patient.
